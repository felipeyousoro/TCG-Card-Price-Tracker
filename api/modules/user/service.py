...
from ...common.exceptions import PermissionDeniedError, UserExistsError, UserNotFoundError, ValidationError
...
class UserService:
    ...
    async def create(self, user: UserCreate, db: AsyncSession) -> dict[str, Any]:
        ...
        Creates a new user with unique username validation. Automatically
    ...
    async def update(self, user_id: int, user_update: UserUpdate, db: AsyncSession) -> dict[str, Any]:
        ...
        Updates user fields with validation for unique constraints on username.
        ...
        Raises:
            ...
            UserExistsError: If username conflicts with existing users.

        Note:
            Validates uniqueness when updating username.
    ...
    async def anonymize_user(self, user_id: int, db: AsyncSession) -> None:
        ...
        Anonymizes user PII while preserving the user record for legal purposes.
        ...
        2. Keeps timestamps for documented legal compliance
        ...
        try:
            ...
            #     extra={
            #         "user_id": user_id,
            #         "retained_data": ["created_at", "updated_at", "id"],
