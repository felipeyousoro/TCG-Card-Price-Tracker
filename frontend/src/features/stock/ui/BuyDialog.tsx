...
import { useBuyCardMutation, useBuyProductMutation } from '../model/mutations'
...
export type BuyTarget =
  | { kind: 'card'; id: number; name: string }
...
export default function BuyDialog({
  ...
  const isPending = buyCard.isPending || buyProduct.isPending
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (target.kind === 'card') {
      buyCard.mutate({ cardId: target.id, payload }, { onSuccess, onError })
    } else {
      buyProduct.mutate({ productId: target.id, payload }, { onSuccess, onError })
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 px-4">
      <div className="w-full max-w-md rounded-xl border border-slate-800 bg-slate-900 p-5 shadow-xl">
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-100">Record buy</h2>
        ...
        <form className="space-y-3" onSubmit={handleSubmit}>
          ...
          <label className="block text-sm">
            ...
            <input name="quantity" type="number" min={1} step={1} required defaultValue={1} className={fieldClass} />
          </label>
          <label className="block text-sm">
